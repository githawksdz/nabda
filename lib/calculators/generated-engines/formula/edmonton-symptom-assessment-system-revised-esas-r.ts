 ...
 * sourceId: calc.edmonton-symptom-assessment-system-revised-esas-r
 * slug: edmonton-symptom-assessment-system-revised-esas-r
 * version: 1.0.0
...
const REQUIRED = ["pain","tiredness","drowsiness","nausea","appetite","breath","depression","anxiety","well"] as const;

export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  slug: "edmonton-symptom-assessment-system-revised-esas-r",
  version: "1.0.0-a25331",
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = ((((((((__num(input["pain"]) + __num(input["tiredness"])) + __num(input["drowsiness"])) + __num(input["nausea"])) + __num(input["appetite"])) + __num(input["breath"])) + __num(input["depression"])) + __num(input["anxiety"])) + __num(input["well"]));
      ...
      const extras = [
        { id: "ESAS-r Score", label: "ESAS-r Score  Clinicians should see a downward trend in ESAS-r score over time with effective treatment.", value: ((((((((__num(input["pain"]) + __num(input["tiredness"])) + __num(input["drowsiness"])) + __num(input["nausea"])) + __num(input["appetite"])) + __num(input["breath"])) + __num(input["depression"])) + __num(input["anxiety"])) + __num(input["well"])) },
      ];
      return {
        ok: true,
        output: {
