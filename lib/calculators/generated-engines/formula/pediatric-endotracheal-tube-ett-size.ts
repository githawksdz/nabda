 ...
 * sourceId: calc.pediatric-endotracheal-tube-ett-size
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      ...
      const extras = [
        { id: "58_Peds ET Tube Size_result", label: "By age 12, most experts recommend switching to adult ETT sizes.", value: " " },
        { id: "58_Peds ET Tube Size_result (uncuffed)", label: "Predicted Uncuffed Tube Size", value: (((Math.floor((__div(__num(input["age"]), 4) + 4)) + "-") + Math.floor((__div(__num(input["age"]), 4) + 4))) + ".5") },
        { id: "58_Peds ET Tube Size_result (uncuffed)", label: "Predicted Uncuffed Tube Size", value: __round((__div(__num(input["age"]), 4) + 4), 1) },
        { id: "mini", label: "Predicted Uncuffed Tube Size", value: (((((__div(__num(input["age"]), 4) + 4) - (Math.floor(__div((__div(__num(input["age"]), 4) + 4), 1)) * 1)) !== 0)) ? ((((Math.floor((__div(__num(input["age"]), 4) + 4)) + "-") + Math.floor((__div(__num(input["age"]), 4) + 4))) + ".5")) : ((__div(__num(input["age"]), 4) + 4))) },
        { id: "58_Peds ET Tube Size_result (cuffed)", label: "Predicted Cuffed Tube Size", value: (((Math.floor((__div(__num(input["age"]), 4) + 3.5)) + "-") + Math.floor((__div(__num(input["age"]), 4) + 3.5))) + ".5") },
        { id: "58_Peds ET Tube Size_result (cuffed)", label: "Predicted Cuffed Tube Size", value: __round((__div(__num(input["age"]), 4) + 3.5), 1) },
      ];
      return {
        ok: true,
        output: {
