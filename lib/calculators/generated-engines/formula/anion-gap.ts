...
const REQUIRED = ["sodium","chloride","bicarbonate","albumin"] as const;

export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  slug: "anion-gap",
  version: "1.0.0-97be48",
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = __round(((__num(input["sodium"]) - __num(input["chloride"])) - __num(input["bicarbonate"])), 1);
      ...
      const extras = [
        { id: "1669_Anion Gap_result (gap)", label: "1669_Anion Gap_result (gap)", value: __round(((__num(input["sodium"]) - __num(input["chloride"])) - __num(input["bicarbonate"])), 1) },
        { id: "1669_Anion Gap_result (corrected)", label: "1669_Anion Gap_result (corrected)", value: __round(((((((__num(input["UOMSYSTEM"]) === false)) ? (__div(__num(input["albumin"]), 10)) : (__num(input["albumin"]))) === 0)) ? ("N/A") : ((((__num(input["sodium"]) - __num(input["chloride"])) - __num(input["bicarbonate"])) + (2.5 * (4 - (((__num(input["UOMSYSTEM"]) === false)) ? (__div(__num(input["albumin"]), 10)) : (__num(input["albumin"])))))))), 1) },
      ];
      return {
        ok: true,
        output: {
          value: primaryValue,
          label: "Anion gap",
          unit: String((((__num(input["UOMSYSTEM"]) === true)) ? ("mEq/L") : ("mmol/L"))),
          extras,
        },
      };
    } catch {
