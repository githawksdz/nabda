 ...
 * sourceId: calc.pulmonary-artery-pulsatility-index-papi
 * slug: pulmonary-artery-pulsatility-index-papi
 * version: 1.0.0
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      ...
      const extras = [
        { id: "10674_PAPi_result", label: "PASP must be greater than PADP. Please check your entries.", value: "" },
        { id: "mini", label: "PAPi", value: __round(__div((__num(input["a"]) - __num(input["b"])), __num(input["c"])), 2) },
        { id: "10674_PAPi_result", label: "PAPi", value: __round(__div((__num(input["a"]) - __num(input["b"])), __num(input["c"])), 2) },
      ];
      return {
        ok: true,
        output: {
          value: primaryValue,
          label: "PASP must be greater than PADP. Please check your entries.",
          unit: "",
          extras,
        },
      };
