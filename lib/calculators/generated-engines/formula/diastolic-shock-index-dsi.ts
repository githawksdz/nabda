 ...
 * sourceId: calc.diastolic-shock-index-dsi
 * slug: diastolic-shock-index-dsi
 * version: 1.0.0
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = __round(__div(__num(input["hr"]), __num(input["dbp"])), 2);
      ...
      const extras = [
        { id: "10673_DSI_result", label: "DSI", value: __round(__div(__num(input["hr"]), __num(input["dbp"])), 2) },
      ];
      return {
        ok: true,
        output: {
