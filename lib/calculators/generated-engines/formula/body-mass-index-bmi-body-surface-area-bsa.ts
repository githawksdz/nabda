 ...
 * sourceId: calc.body-mass-index-bmi-body-surface-area-bsa
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = __round(__div(__num(input["weight"]), Math.pow(__div(__num(input["height"]), 100), 2)), 1);
      ...
      const extras = [
        { id: "29_BMI & BSA_result (BMI)", label: "29_BMI & BSA_result (BMI)", value: __round(__div(__num(input["weight"]), Math.pow(__div(__num(input["height"]), 100), 2)), 1) },
        { id: "29_BMI & BSA_result (BSA)", label: "Body Surface Area", value: __round(Math.pow(__div(((__num(input["weight"]) * __div(__num(input["height"]), 100)) * 100), 3600), 0.5), 2) },
        { id: "29_BMI & BSA_result (target)", label: "29_BMI & BSA_result (target)", value: __round((((__num(input["UOMSYSTEM"]) === true)) ? (__div((__num(input["target"]) * Math.pow(__div(__num(input["height"]), 100), 2)), 0.453592)) : ((__num(input["target"]) * Math.pow(__div(__num(input["height"]), 100), 2)))), 0) },
      ];
      return {
        ok: true,
        output: {
