 ...
 * sourceId: calc.waist-to-hip-ratio
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = __round(__div(((__num(input["UOMSYSTEM"])) ? ((__num(input["waist"]) * 2.54)) : (__num(input["waist"]))), ((__num(input["UOMSYSTEM"])) ? ((__num(input["hip"]) * 2.54)) : (__num(input["hip"])))), 2);
      ...
      const extras = [
        { id: "10658_WHR_result", label: "WHR", value: __round(__div(((__num(input["UOMSYSTEM"])) ? ((__num(input["waist"]) * 2.54)) : (__num(input["waist"]))), ((__num(input["UOMSYSTEM"])) ? ((__num(input["hip"]) * 2.54)) : (__num(input["hip"])))), 2) },
      ];
      return {
        ok: true,
        output: {
