 ...
 * sourceId: calc.radiation-biologically-effective-dose-bed-calculator
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = (__num(input["D"]) * (1 + __div(__num(input["d"]), __num(input["ab"]))));
      ...
      const extras = [
        { id: "10111_Radiation BED Calculator_result (BED)", label: "BED", value: __round((__num(input["D"]) * (1 + __div(__num(input["d"]), __num(input["ab"])))), 2) },
        { id: "10111_Radiation BED Calculator_result (EQD2)", label: "EQD 2 ", value: __round((__num(input["D"]) * __div((__num(input["d"]) + __num(input["ab"])), (2 + __num(input["ab"])))), 2) },
      ];
      return {
        ok: true,
        output: {
