 ...
 * sourceId: calc.digifab-dosing-digoxin-poisoning
...
const REQUIRED = ["choose","weight","dig","amount"] as const;

export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  slug: "digifab-dosing-digoxin-poisoning",
  version: "1.0.0-9e66cc",
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = Math.ceil((((__num(input["choose"]) === 1)) ? ((__div(__num(input["amount"]), 0.5) * 0.8)) : ((((__num(input["choose"]) === 0)) ? (__div(((((__num(input["UOMSYSTEM"]) === true)) ? (__num(input["dig"])) : (__div(__num(input["dig"]), 1.281))) * __num(input["weight"])), 100)) : (0)))));
      ...
      const extras = [
        { id: "4036_DigiFab (Digibind) Dosing_result", label: "DigiFab dose (assumes each vial contains 40 mg)", value: Math.ceil((((__num(input["choose"]) === 1)) ? ((__div(__num(input["amount"]), 0.5) * 0.8)) : ((((__num(input["choose"]) === 0)) ? (__div(((((__num(input["UOMSYSTEM"]) === true)) ? (__num(input["dig"])) : (__div(__num(input["dig"]), 1.281))) * __num(input["weight"])), 100)) : (0))))) },
      ];
      return {
        ok: true,
        output: {
          value: primaryValue,
          label: "DigiFab dose (40 mg vials)",
          unit: "vials",
          extras,
        },
      };
