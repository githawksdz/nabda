 ...
 * sourceId: calc.sedan-score-post-tpa-hemorrhage
...
const REQUIRED = ["gluc","infarct","ct","age","nihss"] as const;

export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  slug: "sedan-score-post-tpa-hemorrhage",
  version: "1.0.0-2867dc",
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = ((((__num(input["gluc"]) + __num(input["infarct"])) + __num(input["ct"])) + __num(input["age"])) + __num(input["nihss"]));
      ...
      const extras = [
        { id: "2039_Sedan Score_result", label: "2039_Sedan Score_result", value: ((((__num(input["gluc"]) + __num(input["infarct"])) + __num(input["ct"])) + __num(input["age"])) + __num(input["nihss"])) },
      ];
      return {
        ok: true,
        output: {
