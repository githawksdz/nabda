 ...
 * sourceId: calc.aortic-dissection-detection-risk-score-add-rs
 * slug: aortic-dissection-detection-risk-score-add-rs
 * version: 1.0.0
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = ((__num(input["hx"]) + __num(input["pain"])) + __num(input["exam"]));
      ...
      const extras = [
        { id: "4060_ADD-RS_result", label: "4060_ADD-RS_result", value: ((__num(input["hx"]) + __num(input["pain"])) + __num(input["exam"])) },
      ];
      return {
        ok: true,
        output: {
