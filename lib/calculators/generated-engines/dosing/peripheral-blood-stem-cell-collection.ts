 ...
 * sourceId: calc.peripheral-blood-stem-cell-collection
...
const REQUIRED = ["wt","blood","peripheral","efficiency"] as const;

export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  slug: "peripheral-blood-stem-cell-collection",
  version: "1.0.0-f5d933",
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = __round(__div(((__div(__num(input["efficiency"]), 100) * __num(input["peripheral"])) * __num(input["blood"])), __num(input["wt"])), 1);
      ...
      const extras = [
        { id: "10009_Peripheral Blood Stem Cell Collection_result", label: "", value: __round(__div(((__div(__num(input["efficiency"]), 100) * __num(input["peripheral"])) * __num(input["blood"])), __num(input["wt"])), 1) },
      ];
      return {
        ok: true,
        output: {
