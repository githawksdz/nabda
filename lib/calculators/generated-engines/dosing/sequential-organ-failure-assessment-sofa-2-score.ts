 ...
 * sourceId: calc.sequential-organ-failure-assessment-sofa-2-score
 * slug: sequential-organ-failure-assessment-sofa-2-score
 * version: 1.0.0
...
export const engine: CalculatorEngine<GeneratedInput, GeneratedOutput> = {
  ...
  calculate(input): CalculatorEngineResult<GeneratedOutput> {
    ...
    try {
      const primaryValue = (((((__num(input["a"]) + __num(input["b"])) + __num(input["c"])) + __num(input["d"])) + __num(input["e"])) + __num(input["f"]));
      ...
      const extras = [
        { id: "10648_SOFA-2 Score_result", label: "SOFA-2 Score", value: (((((__num(input["a"]) + __num(input["b"])) + __num(input["c"])) + __num(input["d"])) + __num(input["e"])) + __num(input["f"])) },
      ];
      return {
        ok: true,
        output: {
