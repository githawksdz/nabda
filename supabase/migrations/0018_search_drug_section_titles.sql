-- Improve drug section title projection for search_documents.
-- Order: sourceHeading → title → heading/sectionTitle → kind → safe plain_text → 'Section'
-- Prefer presentation label when entity_slug differs from identity route slug.

create or replace function public.search_safe_section_title(
  payload jsonb,
  plain_text text
)
returns text
...
as $$
  select coalesce(
    nullif(btrim(payload->>'sourceHeading'), ''),
    nullif(btrim(payload->>'title'), ''),
    nullif(btrim(payload->>'heading'), ''),
    nullif(btrim(payload->>'sectionTitle'), ''),
    nullif(btrim(payload->>'kind'), ''),
    case
      when plain_text is null then null
      when length(split_part(btrim(regexp_replace(plain_text, E'[\n\r]+', E'\n', 'g')), E'\n', 1)) between 1 and 80
        and split_part(btrim(regexp_replace(plain_text, E'[\n\r]+', E'\n', 'g')), E'\n', 1)
          !~* '[<>{}]|mso-|font-size|stylesheet|\.{5,}'
      then split_part(btrim(regexp_replace(plain_text, E'[\n\r]+', E'\n', 'g')), E'\n', 1)
      else null
    end,
    'Section'
  );
...
begin
  ...
  where coalesce(d.slug, '') <> ''
    and (p_prefix = '' or d.slug like p_prefix || '%' or (p_prefix = 'other' and d.slug !~ '^[0-9a-z]'))
  ...
  where l.identity_table = 'drugs'
    ...
    and (
      p_prefix = ''
      or l.payload_entity_slug like p_prefix || '%'
      or (p_prefix = 'other' and l.payload_entity_slug !~ '^[0-9a-z]')
    )
  on conflict (entity_slug) do nothing;

  select count(*) into allowed_count from _search_allowed_drug_slugs;
  ...
  select
    ...
    left(
      concat_ws(
        ' · ',
        coalesce(a.display_title, a.route_slug, s.entity_slug),
        case
          when s.entity_slug is distinct from a.route_slug
          then nullif(regexp_replace(s.entity_slug, '^' || a.route_slug || '-?', ''), '')
          else null
        end,
        public.search_safe_section_title(s.payload, s.plain_text)
      ),
      240
    ),
    ...
    trim(both from concat_ws(
      ' ', a.display_title, a.route_slug, s.entity_slug,
      s.payload->>'sourceHeading', s.payload->>'title', s.payload->>'kind',
    ...
    jsonb_build_object(
      ...
      'payload_item_id', s.payload_item_id,
      'title_source', 'search_safe_section_title'
    ),
    now()
  ...
  select
    ...
    trim(both from concat_ws(
      ' ', a.display_title, a.route_slug, s.entity_slug, s.payload->>'sourceHeading',
...
revoke all on function public.search_safe_section_title(jsonb, text) from public, anon, authenticated;
grant execute on function public.search_safe_section_title(jsonb, text) to service_role;
