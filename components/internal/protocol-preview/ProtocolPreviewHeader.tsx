...
type ProtocolPreviewHeaderProps = {
  title: string;
  sourceId: string;
  backHref: string;
  sectionCount: number;
...
export function ProtocolPreviewHeader({
  title,
  sourceId,
  backHref,
  sectionCount,
}: ProtocolPreviewHeaderProps) {
  return (
    <header className="space-y-3">
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-label-sm text-on-surface-variant">Recommandation · aperçu interne</p>
      ...
      <div className="rounded-xl bg-surface-container-low p-3.5">
        <p className="text-body-sm">Contenu importé depuis nabda_db</p>
        <p className="mt-1 text-body-sm text-on-surface-variant">
          Rendu mobile expérimental. Le texte médical est conservé sans réécriture.
        </p>
        <div className="mt-2 flex flex-wrap gap-1.5">
          <StatusChip label="Importé" />
          <StatusChip label="Non Validé" variant="outline" />
          <StatusChip label={`${sectionCount} sections`} variant="soft" />
        </div>
      </div>
    </header>
  );
